...
{ 
  ...
  auto sizes = cudf::detail::make_counting_transform_iterator(
    ...
    cuda::proclaim_return_type<std::size_t>(
      [gpu_bufs = gpu_bufs.data()] __device__(cudf::size_type i) {
        return gpu_bufs[i].size();
      }
    )
  );
  ...
  auto iter_in = thrust::make_transform_iterator(
    ...
    cuda::proclaim_return_type<thrust::constant_iterator<uint64_t>>(
      [value] __device__ (cudf::size_type i) { 
  ...
  cub::DeviceCopy::Batched(d_temp_storage.data(), temp_storage_bytes, iter_in, iter_out, sizes, num_bufs, stream);
}
